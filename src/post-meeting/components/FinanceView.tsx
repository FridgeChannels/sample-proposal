...
export function FinanceView({ order, onChange, onBack }: { order: OrderState; onChange: (order: OrderState) => void; onBack: () => void }) {
      ...
      <div className="finance-layout">
        <div className="finance-main">
          ...
          <form className="finance-section payment-section" onSubmit={startPayment}>
            ...
            {!locked && <><button className="primary-action pay-invoice" type="submit">Pay Invoice <span>→</span></button><p className="backend-note">Front-end preview only. The secure provider connection is intentionally empty, so this action cannot move funds.</p></>}
          </form>
        </div>

        <aside className="invoice-actions">
          <p className="eyebrow">INVOICE ACTIONS</p><button type="button" className="primary-action" onClick={() => window.print()}>Download Invoice <span>↓</span></button><button type="button" className="secondary-action" onClick={copyPaymentLink}>Copy Payment Link <span>↗</span></button><a className="secondary-action" href={`mailto:?subject=${encodeURIComponent(`Question about invoice ${order.invoiceNumber}`)}`}>Contact FC <span>↗</span></a><button type="button" className="text-button" onClick={onBack}>Back to project order</button><div className="need-changes"><strong>Need changes to this invoice?</strong><p>Finance cannot change package, quantity, scope, unit price, or total. Contact FC to issue a new order version.</p></div>
        </aside>
      </div>
